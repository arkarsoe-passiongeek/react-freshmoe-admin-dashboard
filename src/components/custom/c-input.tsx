...
const CBaseInput = React.forwardRef<HTMLInputElement, CInputProps>(
   ({ className, children, ...rest }, ref) => {
      return (
         <Input
            ref={ref}
            className={`h-[52px] !text-lg placeholder:text-c-contrast ${className}`}
            {...rest}
         />
      );
   }
...
const CInputWithEndButton = React.forwardRef<HTMLInputElement, CInputProps>(
   (
      { className, children, onButtonClick, buttonState = false, ...rest },
      ref
   ) => {
      return (
...
const CSearchInput = React.forwardRef<HTMLInputElement, CInputProps>(
   ({ className, ...rest }, ref) => {
      return (
         <div className='space-y-2'>
            <div className='relative'>
               <Input
                  ref={ref}
                  className={`peer pe-9 ps-9 py-3 rounded-xl h-auto !text-base bg-c-white placeholder:text-c-contrast ${className}`}
                  {...rest}
               />
               <div className='pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 text-muted-foreground/80 peer-disabled:opacity-50'>
                  <Search
                     className='text-c-contrast'
                     size={16}
                     strokeWidth={2}
                  />
               </div>
