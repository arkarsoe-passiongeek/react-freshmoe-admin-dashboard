...
const AccountController = ({ user }: { user: User }) => {
   ...
   const handleLogout = () => {
      const res = logout();
      if (res === 'success') navigate(paths.notAuthorized.path);
   };
      ...
      <>
         <Popover open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
               ...
               <Link onClick={() => setIsOpen(false)} className="flex justify-start border-0 rounded-xl text-c-contrast gap-1 p-3 text-sm items-center hover:bg-c-active-bg hover:text-primary active:bg-c-active-bg active:text-primary" to={paths.profile.path}>
                  <IconUser className="mr-2 w-5 h-5 font-bold" />
                  <span>Profile</span>
               </Link>
               <Link onClick={() => setIsOpen(false)} className="flex justify-start border-0 rounded-xl text-c-contrast gap-1 p-3 text-sm items-center hover:bg-c-active-bg hover:text-primary active:bg-c-active-bg active:text-primary" to={paths.profile.changePassword.path}>
